...
| #11 maintain your own understanding                                            | [`document-code`](../document-code/SKILL.md) makes the code itself durably understandable; [`explain-change`](../explain-change/SKILL.md) ([design spec](grill-to-plan/2026-07-06-post-implementation-understanding-skill/session.md)) covers the session-level half with an explainer plus optional quiz that surfaces gaps in the user's understanding of the change before merging (Shihipar). A shareable buy-in variant — walking reviewers through the same unknowns the author started with — remains deferred until a real team-sharing need appears |
