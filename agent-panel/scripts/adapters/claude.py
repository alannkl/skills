...
class ClaudeAdapter(ProcessAdapter):
    ...
    def command(self, session_id, settings):
        ...
        command = [settings.get('executable', 'claude'), '-p', '--output-format', 'json',
                   ...
                   '--append-system-prompt', 'Perform the task within its declared capabilities. Modify only your own working copy when authorized. Keep source evidence and peer artifacts unchanged. Read peer evidence through the runner-captured artifacts.files and artifacts.patch references after reveal. Bash starts in your working_directory; use that directory for commands. Use file tools for inspection and edits. Keep Bash commands simple: put multiline code, loops, or Unicode test data in a script in your artifact_directory and invoke that file with a task-appropriate executable. Do not embed them in Bash -c arguments or heredocs. Do not commit, push, publish, or launch further agents. Return only the requested JSON.']
