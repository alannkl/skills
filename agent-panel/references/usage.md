...
Both bundled harnesses stream native events to the attempt's `stdout` file while the turn runs. `python3 scripts/progress.py RUN_DIR` prints every participant's reasoning and messages from those files in turn order, unwrapping protocol envelopes to their text or review decision and skipping tool activity; `--follow` polls until the runner writes `report.json`. Claude may redact reasoning to an empty block, which prints nothing. Other adapters' dialects are ignored. `events.jsonl` is appended and fsynced as each turn completes: a `message` event carries the contribution's full text, a `review` event the review, and `phase_end` closes a round.
