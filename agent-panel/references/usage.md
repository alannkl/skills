...
Defaults: three integration/review cycles, 300 seconds of silence per invocation, 300 seconds per check, and 3,600 seconds per discussion. Cancellation/reporting reserves ten seconds, with a five-second minimum. The round cap is the opening phase count plus three times the cycle cap, including restarts after brief changes. These limits bound autonomous work only: they renew for each follow-up and for each host decision at a round boundary, and idle time waiting for a person consumes none of them. There is no whole-chat spending cap. `--unbounded` is an explicit opt-in that removes the discussion deadline until the next host input; the idle bound and round cap still end stalled or looping work, and it is saved like any other limit, so a later `--run-seconds` restores a ceiling. See `--help` for options. There is no cap on a turn's total time. `--idle-seconds` bounds stalls: a turn is killed only after that long with nothing new written under its attempt directory, which both bundled harnesses stream to as they work. Claude streams partial-message deltas, including reasoning, so even a long thinking call keeps the capture growing and silence means a stall; captures are correspondingly larger. Codex emits an event per completed item, so its reasoning phases can be silent for a while. Checks use the same value as a plain wall-clock cap.
...
Both bundled harnesses stream native events to the attempt's `stdout` file while the turn runs. `python3 scripts/progress.py RUN_DIR` prints every participant's reasoning and messages from those files in turn order, unwrapping protocol envelopes to their text or review decision and skipping tool activity; `--follow` polls until the runner writes `report.json`. Claude may redact reasoning to an empty block, which prints nothing; partial-message deltas are ignored by the progress script and count only as activity. Other adapters' dialects are ignored. `events.jsonl` is appended and fsynced as each turn completes: a `message` event carries the contribution's full text, a `review` event the review, and `phase_end` closes a round.
...
`scripts/consult.py` runs one read-only participant outside any panel, for a host that does the work itself; the skill's `pair` mode requires it at fixed checkpoints. `open DIR --harness H --model M [--effort E] [--web] --brief FILE [--read DIR ...]` starts the session and answers the brief; `ask DIR --question FILE [--attach FILE ...] [--read DIR ...]` resumes the same session; `close DIR` ends it. Attached files are inlined into the question; `--read` directories are granted to the harness's file tools, through `--add-dir` for Claude and the read-only sandbox for Codex. The consultant has no write capability in either harness. Each turn's reply is enforced by a one-field schema, so the answer arrives as text, and `--timeout` (default 600 seconds) cancels a turn that never returns while leaving the session open. Stdout carries one JSON object with `ok`, `text` and the turn number; exit 2 means no reply. `DIR/consult.json` records harness, model, session and readable directories; `DIR/log.jsonl` records every question and reply; `DIR/turns/tN/` holds the native captures. `--adapter NAME=FILE` registers external harnesses as for the panel.
