...
## Schema-enforced envelope and peer-nominated integrator, 2026-09-11

- Motivation from session `c1ac32da`: a 14 KB draft envelope from Fable had a JSON syntax error at character 12,264, the draft phase is required, so the discussion ended `incomplete` and the host hand-wrote a follow-up to re-emit it.
- The engine now builds a JSON schema per dispatch: identity and kind pinned by enum, closed objects, every property required, the review schema pinning the exact revision and content hash, the assign schema requiring every roster ID. Claude receives it through `--json-schema` and returns `structured_output`; Codex receives a strict copy through `--output-schema` in which `data` is a JSON-encoded string, decoded by the adapter. Live probes showed Codex rejects an open `data` object (`invalid_json_schema`) and accepts the string form.
- `flat-peers` responsibilities contributions carry an `integrator` nomination. A unanimous nomination by the required peers makes that peer the drafter for the discussion and the runner announces it as a message every peer sees before contributing; split or missing nominations, or a working-copy run that would leave no other approver, fall back to the roster's declared drafter, which stays the saved fallback across follow-ups.
...
## Feature review fixes: paused allowance, adapter idle windows, relay, pair contract, nomination default, 2026-09-11

- Motivation: a pair-mode feature review of the five post-merge commits (GPT-6 Astra consulting) found that the discussion clock renewed on every host decision, including the host agent's own under `--pause-between-rounds`; that 300 seconds of silence is a weak liveness signal for Codex; that verbatim relay of every block, and of every consultant reply, spends the user's attention (measured over 42 captured live turns: reasoning redacted to at most 48 words, messages 400 to 1,755 words per turn); that pair mode closed its consultant at delivery against the chat-persistent contract; and that peer nomination fired blind before proposals were visible. The user decided: keep 3,600 seconds as a ceiling between human interventions, fix the idle bound, fix the relay and pair contracts, keep the declared drafter as the default with unanimous reassignment for a reason, and add plain-name modes with pair as the default mode.
- Clock: a boundary wait pauses the allowance and resumes the remainder; `continue` and `brief` never renew it; only `--continue` starts a fresh allowance. Idle: each bundled adapter declares `idle_seconds` (Claude 300, Codex 600, provisional), external adapters fall back to 300, `--idle-seconds` overrides all and is saved; a null saved limit means adapter windows. Progress: default to the current discussion, `--all` for every discussion, headers carry the attempt, reviews keep revision and hash, `--follow` exits on a finished manifest even when the run ended before it attached. Nomination: participants receive `declared_drafter`; a nomination needs `integrator_reason`; the announcement carries reasons. SKILL.md: pair is a mode with its own prepare and delivery, kept open across follow-ups, two required consultations plus a delivery ledger, conditional retry on failure, concise relay, background execution with notification; plain-name modes; pair by default unless a preset or a panel is requested.
- Seven behavior scenarios were declared before their bodies: paused allowance across `continue` and `brief`, exhaustion across boundaries, adapter idle window versus explicit limit, nomination reason and declared drafter, current-discussion progress with `--all`, follower exit on a finished run, adapter windows surviving follow-ups. The consultant's deliverable review then found that an invalid adapter window was rejected after launch and leaked the running handle; the window is now resolved before any launch, pinned by an eighth scenario. Its other findings were instruction conflicts, all corrected: the Run example no longer overrides adapter windows, reassignment needs every required peer in both documents, retry through `ask` is limited to a failed turn in an existing consultation, plain-name modes map explicitly, `close` is shown as stop or reset only, pair states its own preparation report, follow-up routing is mode-specific, the ledger covers the current task's turns, and general completion duties stay in force for pair. Existing expectations changed for the attempt in progress headers, the review render, nomination reasons, and the follow test's finish signal.
- Final command: `python3 -m unittest discover -s tests/agent-panel`. Result: **89 tests passed, no skips**, in 66 seconds. `git diff --check`, `panel.py --help`, `progress.py --help` and compilation passed. Sensitivity: six targeted mutants (renew at boundary, ignore adapter windows, accept a nomination without a reason, replay every discussion, never finish following, tolerate an invalid window) each made its scenario fail.
- Not exercised live: a full panel under adapter idle windows; the host-level behaviours in pair mode (concise relay, acting on the completion notification) are host instructions that runner tests cannot establish. This change set was itself produced in pair mode: the approach and deliverable checkpoints ran as background commands and the host acted on their completion notifications.
---

Renamed from `agent-panel` to `panel` on 2026-09-11. Earlier entries keep the old paths.
