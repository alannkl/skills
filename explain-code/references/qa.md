...
Answer follow-up questions about the explained code until the user is satisfied.
...
- A question with an embedded hypothesis ("I assume this is for performance?") is a prompt for investigation, not a conclusion to validate: check the code and history first, and disagree plainly when they say otherwise.
...
- The skill's boundaries still hold: explain only — a "should this be refactored?" gets a one-line observation and a suggestion to run a code review, not a critique.
