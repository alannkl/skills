...
- Claude can skip an unapproved tool call without failing the run. It may exit 0 with `is_error: false` even when it completed no work.
...
- `--permission-mode bypassPermissions` (or `--dangerously-skip-permissions`) is the only fully unattended bypass grant.
...
- Use `--output-format json` for programmatic output. Read `.result`, `.session_id`, and `.total_cost_usd`. Add `--json-schema` for typed fields in `.structured_output`.
...
- A `Not logged in` error is fixed by the user running `claude auth login`.
...
- Official docs: <https://code.claude.com/docs/en/headless> and <https://code.claude.com/docs/en/cli-reference>.
