...
- The current directory is the workspace. Cached auth and settings come from `~/.gemini/antigravity-cli/settings.json`.
...
- `--dangerously-skip-permissions` is the fully unattended bypass grant, approving every tool call including writes and commands.

## Bounding

- Always set `--print-timeout <duration>`; otherwise print mode waits up to five minutes. Set the host's timeout slightly higher so the CLI can emit its terminal result first.
...
- Treat every terminal status other than `SUCCESS` as failure: `ERROR`, `CANCELED`, `INTERRUPTED`, `INVALID`, `WAITING`, `RUNNING`.
...
- An `authentication required` error is fixed by the user launching `agy` interactively and completing browser or SSH authentication.
- Install docs: <https://antigravity.google/docs/cli/install>. Its installer is remote; leave running it to the user.
...
- Official docs: <https://antigravity.google/docs/cli/headless>, <https://antigravity.google/docs/cli/permissions>, and <https://antigravity.google/docs/cli/install>.
